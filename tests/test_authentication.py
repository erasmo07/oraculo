...
def test_can_send_message_waboxapp():
    base_url = os.environ.get('WABOXAPP_BASE_URL')
    url_test = 'api/send/chat'

    text = 'ESTO ES UNA PRUEBA DE INTEGRACION'
    params = dict(
        uid=os.environ.get('WABOXAPP_UID'), 
        token=os.environ.get('WABOXAPP_TOKEN'),
        to='18292044821', text=text)
